...
void setup()
{
  ...
  nano.setReadPower(500); //20.00 dBm
...
void loop()
{
  ...
  if (pressed && t_x >=260 && t_y <60 && i>=1){ //up button touched
    i--;
  }
  if (pressed && t_x >=260 && t_y >180 && i < NELEMS(prod)-4){ //down button touched
    i++;
  }
  if (pressed && t_x >=260 && t_y >60 && t_y<180){ //down button touched
    scanTags();
  }
  if (pressed && i>0 && i < NELEMS(prod)-4) drawLayout();
...
void parseJsonResponse(const String& jsonResponse, product* products, size_t arraySize) {
  ...
  JsonArray decodedProducts = jsonDoc["decoded_products"];
  ...
  for (JsonObject productObj : decodedProducts) {
    ...
    products[i].brandName = productObj["manufacturer"];
...
void sendEpcSetToServer(const std::set<String>& epcSet) {
  ...
  JsonArray epcArray = jsonDoc.createNestedArray("epc_codes");
  ...
  const char* serverUrl = "https://smellySocks.pythonanywhere.com/decode_epcs";
...
void scanTags(){ 
  for (;j<1000;j++){ //for prototyping purposes set to 1000 - final product should have ca. 10k reads.
